...
\begin{document}
	...
	\section*{Evaluation of existing software}
		...
		\subsection*{Criteria}
			...
			\todo{Password hashing algorithms strenghts}
		...
		\subsection*{The contestants}
			...
			\subsubsection*{modoboa}
				...
				\ac{ORM}. It passes almost every criterium; just when it comes
			...
			\subsubsection*{PostVis Admin}
				...
				\emph{postifx.admin}}{1.2} but is likely to depart from it in
			...
			\subsubsection*{ratuus}
				...
				\texttt{mysql\_query()}, which in turn supports multiple queries
				configuration dependently. There is an \crit{audit
			...
			\subsubsection*{VBox.Adm}
				...
				sanitizes them. At some places (e.g. in a database maintainance
				script), the queries depend on MySQL. In others (e.g. the
				authentication), they are abstractly noted and \crit{database
				independend}{1.5}. As with most other solutions, there are no
				tests. Solely the custom SMTP proxy for spam maintainance
			...
			\subsubsection*{ViMbAdmin}
				...
				seems not to make use of it conciously (there is no menu on
	...
	\label{sec:moeil}
		...
		intended to be reimplemented normalized \cite{dbnorm} and with a
